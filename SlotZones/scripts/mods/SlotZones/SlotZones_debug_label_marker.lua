--[[
    File: SlotZones_debug_label_marker.lua
    Description: Marker template used for world-marker label rendering.
...
template.name = "SlotZones_debug_label_marker"
template.size = {
	300,
	60,
}
...
local function update_background(widget, ui_renderer, text, font_type, font_size, show_background, alpha)
	...
	if not show_background or not text or text == "" then
		...
		if widget.content then
			widget.content._sz_bg_cache = nil
		end
		return
	end
	...
	local cache = widget.content and widget.content._sz_bg_cache
